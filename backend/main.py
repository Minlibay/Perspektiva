...
ITEM_RULES = {
    ...
    2: {
        ...
        "extra_instructions": (
            ...
            "СРАВНИВАЮТСЯ ТОЛЬКО R_total и P_total. Если они равны — вердикт OK, и точка.\n"
            "НЕ считай несоответствием то, что число КАЛЕНДАРНЫХ дней (P_days) не равно "
            "трудоёмкости в аудитоднях: рабочий день длиннее 8 часов, поэтому 4 календарных дня "
            "штатно дают 4,5 аудитодня. Это нормально, а не нарушение.\n\n"
    ...
    4: {
        ...
        "extra_instructions": (
            ...
            "ПРАВИЛА СОВПАДЕНИЯ:\n"
            "- ИСКЛЮЧЕНИЕ ПО РЕДАКЦИЯМ ТУ: номер изменения ТУ («с изменением 5» vs «с изменением 6») "
            "в файле «Разбивка кодов ОКВЭД» НЕ сравнивай и NOK из-за него НЕ ставь — Разбивка может "
            "законно отставать от актуальной редакции. Редакции ТУ сверяются только между Планом и "
            "Сертификатом (это делает код после твоего ответа).\n"
...
def _count_plan_experts(plan_raw: str) -> int:
    """Число членов экспертной группы в Плане (непустые роли 7.1..7.5).

    ФИО в шаблонах записывают по-разному: «Гревцов К.В.» и «Гревцов Константин
    Викторович» — принимаем оба формата, иначе счёт даёт 0 и детерминированная
    сверка трудоёмкости молча не срабатывает. Пустая роль — «-», «—» или пусто.
    В одной ячейке может быть несколько человек через запятую.
    """
    if not plan_raw:
        return 0
    i = plan_raw.find("Состав экспертной группы")
    if i < 0:
        return 0
    seg = plan_raw[i: i + 2000]
    count = 0
    for m in re.finditer(r"7\.\d+\s+[^|\n]+?\|\s*([^|\n]*)", seg):
        cell = m.group(1).strip(" .;")
        if not cell or cell in ("-", "–", "—"):
            continue
        for name in re.split(r"[,;]", cell):
            name = name.strip()
            # «Фамилия И.О.» / «Фамилия И. О.» либо «Фамилия Имя [Отчество]»
            if re.match(r"^[А-ЯЁ][а-яё]+\s+([А-ЯЁ]\.\s*[А-ЯЁ]\.?|[А-ЯЁ][а-яё]+)", name):
                count += 1
    return count


def _extract_tu_revisions(text: str) -> dict:
    """{номер ТУ (только цифры) → номер изменения} из текста.

    Сплошную формулировку области сертификации сверять дословно бессмысленно:
    Сертификат приходит через OCR, который портит слова и даже реквизиты
    («ОГНО.ВО.0136» вместо «ОГН0.RU.0136»). А номера ТУ и «с изменением N» —
    цифровые, и OCR их сохраняет заметно надёжнее. Ключ нормализуем до цифр,
    чтобы пережить искажения разделителей.
    """
    out = {}
    if not text:
        return out
    for m in re.finditer(
        r"ТУ\s*([\d][\d\.\-\s]{6,40}?)\s*[,\.]?\s*с\s+изменением\s+(\d+)", text, re.I
    ):
        key = re.sub(r"\D", "", m.group(1))
        if len(key) < 12:  # отбрасываем обрывки, которые дал OCR
            continue
    ...
    for idx, item in enumerate(checklist_structure):
        ...
        try:
            ...
            if idx == 2 and plan_raw:
                ...
                # P_experts: непустые роли 7.1..7.5 (принимаются и инициалы, и полное ФИО).
                p_experts = _count_plan_experts(plan_raw)
            ...
            # Пункт 5 (idx=4): область сертификации. Редакции ТУ сверяем только
            # между Планом и Сертификатом: «Разбивка по кодам ОКВЭД» законно
            # отстаёт от актуальной редакции (решение по методике), и её отставание
            # не должно давать NOK. Дословную сверку всей формулировки делать
            # нельзя — Сертификат приходит через OCR и слова искажены.
            if idx == 4 and plan_raw:
                cert_files = [
                    f for f in relevant
                    if any(k in f.lower() for k in ("сертификат", "игс", "макет"))
                ]
                tu_plan = _extract_tu_revisions(plan_raw)
                tu_cert = {}
                for f in cert_files:
                    tu_cert.update(_extract_tu_revisions(all_texts.get(f, "")))

                common = sorted(set(tu_plan) & set(tu_cert))
                if common:
                    diff = [k for k in common if tu_plan[k] != tu_cert[k]]
                    if diff:
                        details = "; ".join(
                            f"ТУ …{k[-8:]}: План изм. {tu_plan[k]} vs Сертификат изм. {tu_cert[k]}"
                            for k in diff
                        )
                        verdict["ok"] = False
                        verdict["nok"] = True
                        verdict["reason"] = (
                            f"Редакции ТУ в Плане и Сертификате расходятся: {details}."
                        )
                    else:
                        verdict["ok"] = True
                        verdict["nok"] = False
                        verdict["reason"] = (
                            f"Область сертификации согласована: редакции ТУ в Плане и "
                            f"Сертификате совпадают ({len(common)} ТУ, изм. "
                            f"{', '.join(tu_plan[k] for k in common)}). Редакции ТУ в "
                            f"«Разбивке по кодам ОКВЭД» в сверку не входят — она может "
                            f"отставать от актуальной редакции."
                        )
                    print(
                        f"[item 5 ТУ] общих={len(common)} расходятся={len(diff)} "
                        f"→ {'NOK' if diff else 'OK'}"
                    )
