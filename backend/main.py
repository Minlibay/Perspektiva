...
def process_checklist_advanced(api_key: str, all_texts: dict,
    ...
    for idx, item in enumerate(checklist_structure):
        ...
        try:
            ...
            # Пункт 11 (idx=10): требуется файл с «СТО» в имени. Детерминированно
            # проверяем без вызова модели — модель в этом пункте часто уходит в рассуждения.
            if idx == 10:
                sto_files = [f for f in file_names if "сто" in f.lower()]
                if not sto_files:
                    checklist_results.append({
                        "ok": False,
                        "nok": True,
                        "reason": (
                            "[авто-NOK: не загружен файл со стандартом «СТО». "
                            "Ожидается отдельный файл с подстрокой «СТО» в имени "
                            "(например, «СТО Газпром 9001-2018»). Без него проверка невозможна.]"
                        ),
                    ...
                    print(f"[item {idx+1}/{total}] NOK (нет файла СТО)")
                    continue
