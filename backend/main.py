...
ITEM_RULES = {
    ...
    1: {
        ...
        "comparison": True,
        "compare_fields": "название заявителя в файле орг.структуры vs в Плане",
        "extra_instructions": (
            "Орг.структура — это графическая схема (квадраты с названиями подразделений). "
            "После OCR получается плоский поток названий в произвольном порядке. Это НОРМАЛЬНО, "
            "иерархию из OCR НЕ восстанавливай и НЕ требуй её совпадения.\n\n"
            "ЕДИНСТВЕННАЯ ПРОВЕРКА: совпадает ли название заявителя в файле орг.структуры "
            "с заявителем из Плана (допускаются сокращения, кавычки, регистр).\n\n"
            "OK — если в файле упомянут тот же заявитель, что в Плане, и видны названия "
            "подразделений (≥3 упоминаний слов «отдел», «управление», «департамент», «служба», «группа», «цех»).\n"
            "NOK — если файл орг.структуры не загружен, ИЛИ заявитель в файле не совпадает с Планом, "
            "ИЛИ это не похоже на орг.структуру (нет упоминаний подразделений).\n\n"
            "Reason: одна короткая строка с заявителем из Плана, заявителем из файла, вердиктом. "
            "Цитаты не нужны. extracted_values НЕ заполняй."
        ),
    },
    2: {
