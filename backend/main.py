...
ITEM_RULES = {
    ...
    1: {
        ...
        "comparison": True,
        "compare_fields": "наличие файла орг.структуры заявителя и базовое соответствие Плану",
        "extra_instructions": (
            "Проверяем НАЛИЧИЕ и БАЗОВУЮ ВАЛИДНОСТЬ файла с организационной структурой заявителя.\n"
            "Источник может называться «Орг.структура», «Организационная структура», «Структура управления» "
            "или сокращённо «ОС <Название компании>».\n\n"
            "ВАЖНО про природу файла: орг.структура почти всегда поставляется как ГРАФИЧЕСКАЯ СХЕМА "
            "(коробочки с названиями подразделений, соединённые линиями). После OCR такая схема "
            "превращается в плоский поток названий подразделений в произвольном порядке — это НОРМАЛЬНО, "
            "иерархия из OCR-текста не восстанавливается. НЕ ТРЕБУЙ дословного совпадения иерархии и "
            "НЕ ставь NOK с формулировкой «не могу сверить иерархию» или «структура не читается».\n\n"
            "АЛГОРИТМ:\n"
            "ШАГ 1. Файл орг.структуры в evidence есть? Если нет ни одного файла с признаками орг.структуры "
            "(в имени или содержимом) → NOK с reason: «Файл орг.структуры заявителя не загружен».\n"
            "ШАГ 2. В файле упомянуто название заявителя (то же, что в шапке Плана, допускаются "
            "сокращения и кавычки)? Если нет → NOK с reason: «В файле орг.структуры не найдено название заявителя <…>».\n"
            "ШАГ 3. В файле перечислено правдоподобное количество подразделений (>= 3 узлов: отделы, "
            "управления, департаменты, службы, цеха, группы и т.п.)? Если нет → NOK с reason: "
            "«Файл не похож на орг.структуру: найдено только <N> упоминаний подразделений».\n"
            "ШАГ 4. Если ШАГИ 1–3 пройдены → OK. В reason укажи: название заявителя из файла, "
            "примерное количество найденных подразделений, и пометку «детальная сверка иерархии "
            "не выполняется — источник графический».\n\n"
            "Заполни extracted_values одной строкой: field='Орг.структура заявителя', "
            "value_in_plan = название заявителя из Плана, value_in_other = название заявителя как "
            "оно фигурирует в файле орг.структуры, other_file = имя файла, "
            "match = true если названия совпадают по существу."
        ),
    },
    2: {
    ...
    6: {
        ...
        "extra_instructions": (
            "Этот пункт — про раздел «4 Наименование и адрес аудитируемых производственных площадок» Плана.\n\n"
            "Проверь по Плану:\n"
            "1. Перечислены ли площадки (центральный офис, производственные площадки, филиалы) с полными адресами.\n"
            "2. Указано ли, является ли структура «объединённой» или каждая площадка идёт отдельно.\n"
            "3. Если упомянута стройка/строительный объект — должно быть указание «Объект заявителя».\n\n"
            "Если в источниках есть файл «Информация для подготовки к аудиту» — дополнительно сверь площадки и адреса с ним.\n\n"
            "OK — если в Плане перечень площадок есть и (при наличии файла «Информация для подготовки») данные не противоречат друг другу.\n"
            "NOK — если в Плане нет ни одной площадки, или есть «стройка» без указания «Объект заявителя», "
            "или (при наличии файла «Информация для подготовки») площадки/адреса расходятся."
        ),
    },
    7: {
        "file_keywords": ["план", "приказ", "эг", "назначен"],
